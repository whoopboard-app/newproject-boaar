@if($ratingSettings && $ratingSettings->apply_to_changelog)
<div class="rating-widget" id="ratingWidget" data-changelog-id="{{ $changelog->id }}">
...
async function submitRating(rating) {
    ...
    const changelogId = document.getElementById('ratingWidget').dataset.changelogId;
    localStorage.setItem(`changelog_rating_${changelogId}`, rating);
    ...
    try {
        const response = await fetch(ratingApiUrl, {
            ...
            body: JSON.stringify({
                rateable_type: 'changelog',
                rateable_id: changelogId,
                rating_value: String(rating),
                rating_type: '{{ $ratingSettings->rating_type }}'
            })
        });
        ...
        if (!response.ok) {
            // If already rated, update the message
            if (response.status === 400 && data.message) {
                feedback.querySelector('.rating-feedback-success span').textContent = data.message;
            }
        }
    } catch (error) {
...
async function submitComment() {
    ...
    // Store in localStorage
    const changelogId = document.getElementById('ratingWidget').dataset.changelogId;
    localStorage.setItem(`changelog_rating_${changelogId}`, 'commented');
...
async function submitAdditionalComment() {
    ...
    // Submit to backend
    try {
        const response = await fetch(ratingCommentApiUrl, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': csrfToken,
...
document.addEventListener('DOMContentLoaded', function() {
    const changelogId = document.getElementById('ratingWidget')?.dataset.changelogId;
    if (changelogId) {
        const ratingKey = `changelog_rating_${changelogId}`;
        ...
        if (existingRating) {
            ...
            if (feedback && options) {
                options.style.display = 'none';
                feedback.style.display = 'block';
                feedback.querySelector('.rating-feedback-success span').textContent = 'You already rated this update. Thank you!';
            }
        }
    }
});
